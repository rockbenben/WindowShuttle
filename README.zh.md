...
**把窗口挪到另一块屏，不该是一次横跨桌面的拖拽。** 按住 `Ctrl` 和右键，往目标屏的方向划一下，光标下那扇窗就已经在那儿了——不用先把「那是第几块屏」想明白。要整块屏一起动？一个快捷键把你所在屏和主屏的窗口全部对调。两种量级的落位都按目标屏重算几何：从 150% 缩放的屏过到 100% 的屏，窗口落地是个合理尺寸，不是原样像素照搬；贴在左半屏的窗口，过去还是左半屏。WindowShuttle 常驻托盘，装上就能用。

- **靠划，不靠瞄。** `Ctrl`+右键，往那块屏一甩，松手。你本来就在朝它比划，不用先把「那是第几块屏」翻译出来。划错了 `Ctrl`+中键 收回来。
- **一扇窗和一整块屏，同一个工具。** 七个动作在同一条量级上：把这一扇送过去、把两块屏整个对调、拔了屏之后把散落的全收回来。键盘、鼠标、托盘菜单、命令行都能触达。
- **几何是重算的，不是照搬的。** 混合 DPI 是主场景不是边缘情况：尺寸按目标屏缩放，最大化仍是最大化，最小化还原到新屏，贴边分屏的窗口过去还占着那半边、那个角。
- **地图就是操作台。** 上半那张图按显示器在你桌上的真实大小和位置绘制——把窗口从一张卡片拖到另一张，它就过去了。
- **不用做任何承诺。** 单文件绿色版，无需安装、无需账号、不联网。仅支持 Windows（靠 `SetWindowPos`/`DeferWindowPos` 直接搬 HWND）。
